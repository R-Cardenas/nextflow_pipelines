...
process seqyclean{
  storeDir "$baseDir/output/cgpMAP/seqyclean"
	input:
	tuple val(read2), file(reads) from read3_ch
	output:
	set val(reads2), file('*_PE1.fastq'), file('*_PE2.fastq') into read4_ch
	script:
	"""
	seqyclean -1 ${reads[0]} -2 ${reads[1]} -qual -gz -o ${read2}
	"""
}
...
process cgpMAP {
  ...
  input:
  tuple val(read2), file(reads) from read4_ch
